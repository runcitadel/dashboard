<template>
  <card-widget
    header="Liquid Wallet"
    :status="{
      text: liquidStore.percent < 100 ? 'Synchronizing' : 'Active',
      variant: 'success',
      blink: false,
    }"
    :sub-title="$filters.formatUnit(systemStore.unit)"
    icon="icon-app-liquid.svg"
    :loading="false"
  >
    <template #title>
      <div
        ...
        :title="$filters.satsToUSD(123456, bitcoinStore).toString()"
      >
        <CountUp
          :value="{
            endVal: 123456,
        ...
        <div v-else-if="mode === 'deposit'" class="wallet-mode mode-deposit">
          <div class="px-3 px-lg-4">
            ...
            <p class="text-center text-muted mb-3">
              <span>
                Send
                <b>only LiquidBitcoin</b> to this address
              </span>
            </p>
...
import CircularCheckmark from './Utility/CircularCheckmark.vue';
...
export default defineComponent({
  ...
  methods: {
    ...
    async changeMode(
      ...
      //change between different modes/screens of the wallet from - transactions (default), withdraw, withdrawing, deposit
    ...
    reset() {
      ...
      //in case going back from review withdrawal to edit withdrawal
